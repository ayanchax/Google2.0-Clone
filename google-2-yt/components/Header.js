...
function Header() {
    ...
    const [searchInput, setSearchInput] = useState(router.query?.term ? router.query?.term : null);
        ...
        <header className="sticky top-0 bg-white">
            <div className="flex w-full p-6 items-center">
                ...
                <form className="flex flex-grow  px-6 py-3 ml-10 mr-5 border border-gray-200 rounded-full shadow-lg max-w-3xl items-center">
                    <input value={searchInput} onChange={updateSearchHandler}
                        ref={searchInputRef}
                        type="text"
                ...
                <Avatar className="ml-auto" url={AVATAR_URL} />
            </div>
            <HeaderOptions />
        </header>
    );
